...
| --- | --- | --- | --- | --- |
| user\_roles | id | INT | PK | e.g., 1=GUEST, 2=FREE, 3=BASIC, 4=AUTHOR |
...
| version | INT | DEAFAULT 0 NOT NULL | Optimistic locking for Hibernate (@Version) |
...
| --- | --- | --- | --- |
| id | UUID | PK | Unique identifier |
| author\_id | UUID | **Soft Link** | Logical link to user\_schema.users(id)  B-tree index |
...
| published\_at | TIMESTAMP WITH TIME ZONE | NULLABLE | Used for sorting feed (Needs Index) |
...
| --- | --- | --- | --- |
| id | UUID | PK | Unique identifier |
| post\_id | UUID | FK -> posts(id) | Associated post |
...
| --- | --- | --- | --- |
| id | UUID | PK | Unique identifier |
| room\_id | UUID | FK -> rooms(id) | Chat session ID |
